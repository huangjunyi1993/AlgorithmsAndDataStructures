 ...
 */
public class SizeBalancedTree02<K extends Comparable<K>, V> {
    ...
    public K getIndexKey(int index) {
        ...
        return getIndex(this.root, index + 1).key;
    }

    private SizeBalancedTreeNode<K, V> getIndex(SizeBalancedTreeNode<K, V> cur, int size) {
        if (size == (cur.left != null ? cur.left.size : 0) + 1) {
            return cur;
        } else if (size >= (cur.left != null ? cur.left.size : 0)) {
            return getIndex(cur.left, size);
        } else {
            return getIndex(cur.right, size - (cur.left != null ? cur.left.size : 0) - 1);
        }
    }
