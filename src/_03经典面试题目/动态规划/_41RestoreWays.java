...
public class _41RestoreWays {
    ...
    /**
     * i位置的数，把它变成V，S是i位置的数与右边数的关系，返回把数组变成达标数组，有多少中方法
     * @param arr 原数组
     * @param i 当前下标
     * @param V 把i位置的数变为V
     * @param S 与右侧数的关系， 0表示小于右边的数，1表示等于右边的数，2表示大于右边的数
     * @return
     */
            ...
            for (int V = 1; V < 201; V++) {
                for (int S = 0; S < 3; S++) {
                    if (arr[i] != 0 && arr[i] != V) {
                        ...
                        int ways = 0;
                        if (S == 0 || S == 1) {
                            ways += preSum[V - 1][0] + dp[i - 1][V][1] + preSum[200][2] - preSum[V][2];
                        } else {
