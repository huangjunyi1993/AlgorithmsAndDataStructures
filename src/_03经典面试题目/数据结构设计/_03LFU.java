...
public class _03LFU {
    ...
    private static class Node {
        ...
        public Node(int times, int key, int value) {
            this.times = times;
            this.key = key;
            this.value = value;
        }
    ...
    private static class NodeList {
        ...
        private static class LFUCache {
            ...
            private Map<Node, NodeList> nodeListMap;
            ...
            public void put(int key, int value) {
                if (nodeMap.containsKey(key)) {
                    ...
                    if (size == capacity) {
                        Node head = headList.head;
                        headList.removeNode(head);
                        modifyNodeList(headList);
                        nodeMap.remove(head.key);
                        nodeListMap.remove(head);
                        size--;
                    }
            ...
            private boolean modifyNodeList(NodeList nodeList) {
                if (nodeList.isEmpty()) {
                    if (headList == nodeList) {
                        ...
                        if (nodeList.next != null) nodeList.next.last = nodeList.last;
                    }
                    return true;
                }
                return false;
            ...
            private void move(Node node, NodeList nodeList) {
                nodeList.removeNode(node);
                NodeList preList = modifyNodeList(nodeList) ? nodeList.last : nodeList;
                NodeList nextList = nodeList.next;
                if (nodeList == null) {
                    NodeList newList = new NodeList(node);
                    newList.last = preList;
                    if (preList != null) preList.next = newList;
                    if (headList == null) headList = newList;
                    nodeListMap.put(node, nodeList);
                } else {
                    if (nextList.head.times == node.times) {
                        nextList.addNode(node);
                        nodeListMap.put(node, nextList);
                    } else {
                        NodeList newList = new NodeList(node);
