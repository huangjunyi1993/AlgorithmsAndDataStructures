 ...
 */
public class Code23_01AddValue {
    public static int max(String s) {
        ...
        return process(arr, 1, arr[0], 1);
    }

    private static int process(int[] arr, int index, int lastNum, int baseValue) {
        // base case
        if (index == arr.length) return 0;
        // 不要
        int p1 = process(arr, index + 1, lastNum, baseValue);
        // 要
        int p2 = process(arr, index + 1, arr[index], arr[index] == lastNum ? baseValue + 1 : 1);
        // PK
