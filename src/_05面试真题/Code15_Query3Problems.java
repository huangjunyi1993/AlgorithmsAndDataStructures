 ...
 * 问题1：用前缀和数组sum1求解
 *
 * 问题2：平方前缀和数组，假设数组[a, b, c, d] => [a^2, b^2, c^2, d^2] => 平方前缀和数组sum2
 * 假设[a,b,c,d]，a+b+c+d = s
 * (s-a)^2 + (s-b)^2 + (s-c)^2 + (s-d)^2
 * 4s^2 - 2s(a + b + c + d) + a^2 + b^2 + c^2 + d^2
 * 4s^2 - 2s^2 + a^2 + b^2 + c^2 + d^2
 * 2s^2 + a^2 + b^2 + c^2 + d^2
 * 公式：假设L~R有k个数 (k - 2)s^2 + a^2 + b^2 + ...
 * s 可以通过sum1得到
 * a^2 + b^2 + ... 可以通过sum2得到
 *
 * 问题3：线段树求范围最大值
 */
public class Code15_Query3Problems {
