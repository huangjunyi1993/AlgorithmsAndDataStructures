 ...
 */
public class Code25_SequenceKDifferentKinds {
    public static int numsMe(String s, int k) {
        int n = s.length();
        ...
        return process(charCount, 0, k);
    }

    /**
     * charCount从index位置开始（charCount[index......]），搞定rest中字符，有多少种方法
     * @param charCount 词频数组
     * @param index 词频数组当前下标
     * @param rest 剩余要搞定的字符种数
     * @return
     */
    private static int process(int[] charCount, int index, int rest) {
        // base caes，rest==0，返回1种方法，否则0种
        if (index == charCount.length) return rest == 0 ? 1 : 0;
        int p1 = process(charCount, index + 1, rest);
        int p2 = 0;
        if (rest != 0) {
            int next = process(charCount, index + 1, rest - 1);
            // 当前字符种数假设n，可以凑出多少组？
            // 自己单独一组，n组
            // 两两一组，cn2
            // 三三一组，cn3
