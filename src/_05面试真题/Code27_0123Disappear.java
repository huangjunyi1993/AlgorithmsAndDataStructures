 ...
 */
public class Code27_0123Disappear {

    public static int maxDisappearMe(String str) {
        if (str == null || str.length() == 0) return 0;
        return disappearMe(str.toCharArray(), 0, str.length() - 1);
    }

    /**
     * 字符串chs，范围从l到r，最大的全消子序列是多长
     * @param chs 字符串
     * @param l 范围左边界
     * @param r 范围右边界
     * @return
     */
    public static int disappearMe(char[] chs, int l, int r) {
        if (l >= r) return 0;
        if (l + 1 == r) return ((chs[l] == '0' && chs[r] == '1') || (chs[l] == '2' && chs[r] == '3')) ? 2 : 0;
        // 不要l
        int p1 = disappearMe(chs, l + 1, r);
        if (chs[l] == '1' || chs[l] == '3') return p1;
        // 要l
        char find = chs[l] == '0' ? '1' : '3';
        int p2 = 0;
        for (int i = l + 1; i <= r; i++) {
            if (chs[i] == find) {
                // [L[L+1...i-1]i][i+1 ... r]
                p2 = Math.max(p2, disappearMe(chs, l+1, i-1) + 2 + disappearMe(chs, i + 1, r));
            }
        }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(maxDisappearMe(str1));

        String str2 = "021331";
        System.out.println(maxDisappear(str2));
        System.out.println(maxDisappearMe(str2));
    }
}
